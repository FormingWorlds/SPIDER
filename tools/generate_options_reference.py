...
-----
    ...
    python tools/generate_options_reference.py --check

The committed page is regenerated by the documentation workflow at
deploy time, and CI verifies with ``--check`` that the committed copy
matches what parameters.c currently produces, so the two cannot drift.
Run this script and commit the result whenever options change.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        '--check',
        action='store_true',
        help='Verify the committed page matches parameters.c; exit 1 on drift.',
    )
    ...
    if args.check:
        ...
        if committed != rendered:
            print(
                f'{args.out} is out of date with parameters.c; regenerate it with\n'
                '  python tools/generate_options_reference.py'
            )
            return 1
        print(f'{args.out} is current ({len(rows)} options)')
        return 0
