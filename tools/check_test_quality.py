#!/usr/bin/env python3
"""AST-based test-quality linter for the SPIDER test suite.

Enforces the rules in `.github/.claude/rules/spider-tests.md` (sections 1 + 7):
...
- Forbidden weak assertions when they stand alone as the sole meaningful
  ...
  three-class discrimination guard from spider-tests.md section 2 uses
...
- Optional dependencies (``matplotlib``, ``aragog``, ``bibtexparser``) imported at
  module top without a preceding ``pytest.importorskip('<name>')`` (the minimal
  CI Python environment otherwise fails collection).
...
# SPIDER's C sources live flat at the repo root; there is no src/ package.
...
# scope, otherwise the minimal CI Python environment fails collection.
# Source rule: spider-tests.md section 6.
OPTIONAL_DEPS = {
    'matplotlib',
    'aragog',
    'bibtexparser',
...
def reference_pinned_status() -> list[str]:
    ...
    for source in sorted(PHYSICS_SOURCES):
        # source is e.g. 'twophase.c' -> companion is tests/test_twophase.py
...
def cmd_baseline() -> int:
    ...
    allow = os.environ.get('SPIDER_TEST_QUALITY_ALLOW_REGRESS') == '1'
    if old and new_total > old_total and not allow:
        print(
            ...
            f'pre-existing violations), set SPIDER_TEST_QUALITY_ALLOW_REGRESS=1.',
            file=sys.stderr,
        )
        return 2
