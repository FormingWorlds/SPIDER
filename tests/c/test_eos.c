static const char help[] =
    "Evaluates a single SPIDER EOS at probe points given in SI units.\n"
    ...
    "         -eos_type <lookup|adamswilliamson> (default lookup)\n"
    ...
    "for any choice of scalings. The Adams-Williamson EOS defines only the\n"
    "density, so only rho is printed for it. Prints one JSON object to\n"
    "stdout; the pytest wrapper owns all assertions.\n";
...
int main(int argc, char **argv)
{
  ...
  ierr = PetscStrcmp(type, SPIDER_EOS_LOOKUP, &is_lookup);CHKERRQ(ierr);
  ierr = EOSCreate(&eos, is_lookup ? SPIDER_EOS_LOOKUP : SPIDER_EOS_ADAMSWILLIAMSON);CHKERRQ(ierr);
  ...
  for (i = 0; i < nP; ++i) {
    ...
    if (is_lookup) {
      ...
      log10visc[i] = eval.log10visc + SC->LOG10VISC;
      cond[i]      = eval.cond * SC->COND;
  ...
  ierr = SpiderTestPrintScalarArray("rho", rho, nP, is_lookup);CHKERRQ(ierr);
  if (is_lookup) {
    ...
    ierr = SpiderTestPrintScalarArray("log10visc", log10visc, nP, PETSC_TRUE);CHKERRQ(ierr);
    ierr = SpiderTestPrintScalarArray("cond", cond, nP, PETSC_TRUE);CHKERRQ(ierr);
